...
export default function BalanceBar({ balances, currentMemberId }: BalanceBarProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 sm:gap-3">
      {/* You Owe Card */}
      <div className="bg-surface border border-border rounded-md p-3.5 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] flex flex-col justify-between min-h-[130px] sm:min-h-[140px]">
        <p className="font-ui text-[10px] sm:text-[11px] font-medium uppercase tracking-[0.1em] text-text-muted">
          You owe
        </p>
        <div>
          <div className="flex items-baseline gap-1 mt-2">
            <span className="font-display text-[36px] sm:text-[48px] font-light leading-none text-text-primary">
              {Math.floor(youOwe / 100)}
            </span>
            <span className="font-ui text-[12px] sm:text-[14px] text-text-secondary">
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex -space-x-2">
              {creditors.slice(0, 3).map((m) => (
                <MemberAvatar key={m.id} name={m.display_name} color={m.avatar_color} size={24} />
              ))}
            </div>
      ...
      <div className="bg-surface border border-border rounded-md p-3.5 sm:p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06)] flex flex-col justify-between min-h-[130px] sm:min-h-[140px]">
        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <svg className="w-4 h-4 sm:w-5 sm:h-5 text-positive" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M7 17L17 7M17 7H7M17 7V17" />
            </svg>
          </div>
        </div>
